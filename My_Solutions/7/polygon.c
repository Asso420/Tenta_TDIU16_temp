...
e) Ja, för att det två semaphorer en som signalerar när trådarna ska börja jobba och en 
när de ska sluta jobba, och även de kretiska områden är skyddade med lås. men implementereringen 
går inte att komplera då, det finns ställen som jag inte kunnat synkronserat helt. 
    ...
    while (true) {
        ...
        for (int x = 0; x < temp->width; x++) {
            for (int y = 0; y < temp->height; y++) {
                ...
                //lock_acquire(&p->coor_lock);
                if (point_in_poly(p, poly.points, poly.count)) {
                    temp->cells[x][y] = 1;
                    //lock_release(&p->coor_lock);
                } else {
                    temp->cells[x][y] = 0;
                   // lock_release(&p->coor_lock);
                }
